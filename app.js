...
// produces a 5-session (one trading week on Tadawul: Sun–Thu) trend
// forecast. Re-renders when live data arrives.
(function () {
  ...
  function buildForecast(candles, patterns, ind) {
    ...
    var rationale =
      ...
      "Per Bulkowski's statistics, the dominant pattern's average post-breakout move feeds the projected target. " +
      "Key drivers: " + bullets + ".";

    return {
      direction: direction, label: label, score: score, confidence: confidence,
      target: target, lower: lower, upper: upper, rationale: rationale
    };
  }
